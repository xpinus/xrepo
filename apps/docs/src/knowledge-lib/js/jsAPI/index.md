...
<run-script  :code="map.js"></run-script>
...
<run-script name="哪段代码效率更高" :code="q1"></run-script>

指导代码习惯：定义类或对象时，尽可能保证顺序一致

## 数组的快速模式和字典模式

<run-script name="哪段代码效率更高" :code="q2"></run-script>
...
import map from './src/map.js?raw';
import clipboard from './src/clipboard.js?raw';
import receiver from './src/receiver.js?raw';
</script>
